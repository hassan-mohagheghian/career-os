...
function PrefForm({ initial, onSave, onCancel }) {
...
function SortableRule({ pref, idx, editing, onEdit, onSave, onCancel, onToggle, onDelete, onPriority }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={cn("rounded-lg transition hover:bg-muted/50 group", !pref.enabled && "opacity-40", isDragging && "ring-2 ring-primary/30 shadow-lg")}>
      {editing === pref.id ? (
        <div className="p-1">
          <PrefForm initial={pref} onSave={(form) => onSave(pref.id, form)} onCancel={onCancel} />
        </div>
      ) : (
  ...
  return (
    <div className="flex flex-col min-w-0">
      ...
      <div className={cn("rounded-b-lg border p-2 space-y-0.5", meta.headerBorder)}>
        ...
        {/* Add form */}
        {showAdd === type && (
          <PrefForm initial={{ category: type, key: '', value: '', description: '', priority: 50 }} onSave={(form) => { onAdd(form); setShowAdd(null) }} onCancel={() => setShowAdd(null)} />
        )}
        {/* Rules list — sortable */}
...
export default function PreferencesTab({ preferences, onUpdate }) {
  ...
  const handleSave = async (id, form) => { await api('PUT', `/preferences/${id}`, { value: form.value, description: form.description }); setEditing(null) }
  const handleToggle = async (id, enabled) => api('PUT', `/preferences/${id}`, { enabled: enabled ? 1 : 0 })
  const handleDelete = async (id) => api('DELETE', `/preferences/${id}`)
  const handleAdd = async (form) => { await api('POST', '/preferences', { preferences: [form] }) }
  const handlePriority = async (id, delta, current) => api('PUT', `/preferences/${id}`, { priority: Math.max(0, Math.min(100, current + delta)) })

  const handleReorder = async (type, reordered) => {
    // Assign new priorities based on position: top = 100, bottom = decreasing
    const total = reordered.length
    const updates = reordered.map((pref, i) => {
      const newPriority = Math.max(1, Math.round(100 - (i / Math.max(total - 1, 1)) * 99))
      return api('PUT', `/preferences/${pref.id}`, { priority: newPriority })
    })
    await Promise.all(updates)
  }

  if (!preferences) return <div className="text-center py-12 text-muted-foreground">Loading rules...</div>

  const fitPrefs = (preferences.fit || []).sort((a, b) => b.priority - a.priority)
  const succPrefs = (preferences.success || []).sort((a, b) => b.priority - a.priority)
  const total = fitPrefs.length + succPrefs.length
  const enabled = fitPrefs.filter(p => p.enabled).length + succPrefs.filter(p => p.enabled).length
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <RuleColumn type="fit" prefs={fitPrefs} editing={editing} showAdd={showAdd} setShowAdd={setShowAdd}
        ...
        <RuleColumn type="success" prefs={succPrefs} editing={editing} showAdd={showAdd} setShowAdd={setShowAdd}
          onEdit={setEditing} onSave={handleSave} onCancel={() => setEditing(null)}
          onToggle={handleToggle} onDelete={handleDelete} onPriority={handlePriority} onAdd={handleAdd} onReorder={handleReorder} />
      </div>
    </div>
