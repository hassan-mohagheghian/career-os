...
function RoadmapNode({ item, checked, onToggle, depth = 0 }) {
...
export default function SkillRoadmapDrawer({
  skillName,
  open,
  onOpenChange,
  onRefreshProgress,
  ...
  const fetchRoadmap = useCallback(async () => {
    ...
    try {
      const [roadmapItemsRes, progressRes] = await Promise.all([
        ...
        fetch(
          `${API}/skill-roadmap-progress?skill=${encodeURIComponent(skillName)}`,
        ),
      ]);
  ...
  const fetchGenProgress = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API}/skill-roadmaps/progress?skill=${encodeURIComponent(skillName)}`,
      );
  ...
  }, [fetchRoadmap, fetchGenProgress, open]);

  const handleToggle = async (itemId, completed, childIds = []) => {
    // Optimistically update all
    setChecked((prev) => {
      const next = { ...prev, [itemId]: completed };
    ...
    try {
      ...
      // Revert on error
      setChecked((prev) => {
        const next = { ...prev, [itemId]: completed ? 0 : 1 };
  ...
  const total = countLeafItems(roadmapItems);
  ...
  const genPct = genProgress
    ? Math.round(
        ((genProgress.step || 0) / (genProgress.total_steps || 4)) * 100,
      )
    : 0;
