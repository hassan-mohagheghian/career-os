      ...
      <div className="flex-1 space-y-1">
        <div className="h-[3px] rounded-full bg-muted">
          <div
            ...
            style={{ width: `${skill.demandPercentage || 0}%` }}
          />
        </div>
        ...
        {hasProgress ? (
          ...
          <Badge
            ...
            className={cn(
              ...
              levelColors[skill.candidateLevel || skill.currentLevel] ||
                "bg-gray-500/15 text-gray-400",
            )}
          >
...
export default function SkillsIntelSection({
  ...
  // Fetch tech_stack to show ALL skills (even without roadmaps)
  useEffect(() => {
  ...
  const aiSkills = [
    ...new Set(
      [...strengths, ...gaps, ...recommendations.map((r) => r.skill)].map(
        (s) => s.skill || s,
      ),
    ),
  ];
  ...
  const allSkillNames = [
    ...new Set([...techStackSkills.map((s) => s.name), ...aiSkills]),
  ];
  ...
  const handleAddCustomSkill = async () => {
    ...
    try {
      ...
      // Open the drawer for this skill to generate roadmap
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          size="sm"
          onClick={onRefresh}
          disabled={refreshing.skills}
          className="gap-1 h-6 text-[0.55rem]"
        >
          <ArrowsClockwise
            className={cn("w-3 h-3", refreshing.skills && "animate-spin")}
          />{" "}
          Refresh
        </Button>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          {
            n: recommendations.filter((r) => r.roi >= 8).length,
            l: "High ROI",
            c: "text-yellow-500",
          ...
          <Card
            key={i}
            className="p-3 text-center transition hover:border-primary"
          >
      ...
      <Card className="p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <Badge
            variant="secondary"
            className="text-[0.5rem] bg-emerald-500/15 text-emerald-500"
          >
        ...
        <div className="space-y-2">
          {allSkillNames.map((skill) => {
            ...
            const isUser = techStackSkills.some(
              (s) => s.name === skill && s.source === "user",
            );
            ...
            return (
              ...
              >
                <span className="text-xs font-semibold w-24 truncate">
                  {skill}
                </span>
                {isUser && (
                  <Badge
                    variant="secondary"
                    className="text-[0.4rem] h-3.5 bg-purple-500/15 text-purple-500 shrink-0 gap-0.5"
                  >
      ...
      {allSkillNames.filter((s) => {
        ...
        <Card className="p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <Badge
              variant="secondary"
              className="text-[0.5rem] bg-purple-500/15 text-purple-500"
            >
              {
                allSkillNames.filter((s) => {
                  const prog = roadmapProgress[s];
              ...
              .map((skill) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[0.6rem] text-muted-foreground flex-1">
                      No roadmap yet
                    </span>
                    <Button
                      size="sm"
        ...
        <div className="space-y-4">
          ...
          <Card className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <Badge
                variant="secondary"
                className="text-[0.5rem] bg-green-500/15 text-green-500"
              >
                {strengths.length}
              </Badge>
            </div>
          ...
          <Card className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <Badge
                variant="secondary"
                className="text-[0.5rem] bg-red-500/15 text-red-500"
              >
                {gaps.length}
              </Badge>
            </div>
