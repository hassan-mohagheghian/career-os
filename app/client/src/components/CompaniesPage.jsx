          ...
          <div className="rounded border p-1.5 shrink-0 mb-1 bg-muted min-w-0">
            {editingId && (
              <div className="flex items-center gap-1 mb-1 text-[0.5rem] text-primary">
                ...
                <span>Adding notes to pending #{editingId}</span>
            ...
            <div className="flex gap-1">
              <Textarea value={noteInput} onChange={e => { setNoteInput(e.target.value); setError('') }}
                ...
                placeholder="Add a note: URL, company name, description, anything about the company..."
                className="w-full h-10 rounded border text-[0.6rem] min-w-0 resize-none flex-1" />
              <Button onClick={addNote} disabled={!noteInput.trim()} size="sm" variant="outline" className="h-10 px-2 shrink-0">
            ...
            <div className="flex items-center gap-1 mt-1">
              <Button onClick={handleSubmit} disabled={submitting || (notes.length === 0 && !noteInput.trim())} size="sm" className="flex-1 h-6 text-[0.55rem]">
                {submitting ? '...' : editingId ? 'Add Notes' : processImmediately ? 'Add & Process' : 'Add'}
              </Button>
              {!editingId && (
                <button
