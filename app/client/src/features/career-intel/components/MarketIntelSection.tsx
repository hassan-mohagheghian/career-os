  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <Button variant="ghost" size="sm" onClick={onRefresh} disabled={refreshing.market} className="gap-1 h-6 text-2xs">
              ...
              {cities.map((city, i) => (
                <div key={i} className="p-2 rounded-lg border border-muted hover:border-primary transition">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary" className={cn("text-2xs h-3",
                  ...
                  <div className="text-2xs text-muted-foreground mt-0.5">{city.jobCount} jobs</div>
                  {city.topCompanies?.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {city.topCompanies.slice(0, 3).map((co, j) => (
                        <Badge key={j} variant="secondary" className="text-2xs">{co}</Badge>
                      ))}
                    </div>
                  )}
                  {city.insight && <div className="text-2xs text-primary mt-1">{city.insight}</div>}
                </div>
              ))}
            </div>
          </Card>
              ...
              <div className="space-y-1.5">
                {countries.map((c, i) => (
                  <div key={i} className="flex items-center gap-1.5">
                    <div className="w-16 text-2xs text-muted-foreground truncate">{c.name}</div>
                    <div className="flex-1 h-3.5 bg-muted rounded overflow-hidden">
                      <div className="h-full bg-primary/60 rounded flex items-center px-1" style={{ width: `${Math.max(c.percentage || 5, 5)}%` }}>
                        <span className="text-2xs font-bold text-white">{c.jobCount}</span>
                      </div>
                    </div>
                    <div className="w-8 text-right text-2xs font-bold text-primary">{Math.round(c.percentage || 0)}%</div>
                  </div>
                ))}
              </div>
            </Card>
          ...
          {remote.count > 0 && (
            <Card className="p-3">
              ...
              <div className="text-center mb-2">
                ...
                <div className="text-2xs text-muted-foreground">{Math.round(remote.percentage || 0)}% of all jobs</div>
              </div>
              {remote.topCompanies?.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {remote.topCompanies.slice(0, 4).map((co, i) => (
                    <Badge key={i} variant="secondary" className="text-3xs">{co}</Badge>
                  ))}
                </div>
              )}
            </Card>
              ...
              <div className="space-y-1.5">
                {visa.map((v, i) => (
                  <div key={i} className="flex items-center gap-1.5 text-2xs">
                    ...
                    <Badge variant="secondary" className={cn("text-3xs h-2.5 shrink-0",
              ...
              <div className="space-y-1.5">
                {insights.map((insight, i) => (
                  <div key={i} className="p-1.5 rounded border border-yellow-500/20 bg-yellow-500/5">
                    ...
                    {insight.action && <div className="text-2xs text-primary mt-0.5">{insight.action}</div>}
                  </div>
                ))}
              </div>
            </Card>
