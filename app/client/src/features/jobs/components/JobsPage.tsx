...
export default function JobsPage({
  ...
  rescoreJob, deleteJob, requeueJob, openDrawer, refreshJobs, clearFilters, loadMoreJobs, onOpenCompany, openWorkflow,
  onOpenQueueDrawer, onOpenAddJobDrawer,
}) {
  const processingCount = pending.filter(p => p.status === 'queued' || p.status === 'processing' || p.status === 'pending').length

  return (
    <div className="flex flex-col h-[calc(100vh-80px)] gap-2">
      <PageHeader title="Jobs" className="px-1">
        <Button variant="outline" size="sm" className="h-7 text-xs gap-1 relative" onClick={onOpenQueueDrawer}>
          <List className="w-3.5 h-3.5" />
          Queue
          {processingCount > 0 && (
            <Badge variant="default" className="text-2xs h-4 px-1 absolute -top-1.5 -right-1.5">{processingCount}</Badge>
          )}
        </Button>
        <Button size="sm" className="h-7 text-xs gap-1" onClick={onOpenAddJobDrawer}>
      ...
      {/* Jobs List */}
      <div className="flex flex-col rounded-lg border overflow-hidden bg-card flex-1 min-h-0">
        <div className="px-2 py-1.5 flex items-center gap-1 shrink-0 bg-gradient-to-r from-green-500/10 to-green-500/5 border-b border-green-500/20">
          ...
          <span className="font-bold text-xs text-green-500">Jobs List</span>
