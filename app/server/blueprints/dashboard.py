...
def create_tech_learning():
    ...
    cur = conn.execute(
        ...
        (data['name'], data.get('priority', 100), data.get('usage', 0),
         data.get('reason', ''), data.get('action', ''))
    )
    conn.commit()
    row = conn.execute('SELECT * FROM tech_learning WHERE id=?', (cur.lastrowid,)).fetchone()
    conn.close()
...
def create_tech_stack():
    ...
    existing = conn.execute('SELECT id FROM tech_stack WHERE name=?', (data['name'],)).fetchone()
    if existing:
        conn.close()
        return jsonify({'id': existing[0], 'name': data['name'], 'message': 'Skill already exists'}), 200
    cur = conn.execute(
        'INSERT INTO tech_stack (name, level, roles, path, source) VALUES (?, ?, ?, ?, ?)',
        (data['name'], data.get('level', 1), data.get('roles', ''), data.get('path', ''), data.get('source', 'user'))
    )
    conn.commit()
    row = conn.execute('SELECT * FROM tech_stack WHERE id=?', (cur.lastrowid,)).fetchone()
    conn.close()
    return jsonify(dict(row)), 201
...
# ═══════════════════════════════════════════════════════
# Skill Topics — hierarchical learning tree per skill
# ═══════════════════════════════════════════════════════
...
def create_skill_roadmap():
    ...
    for item in items:
        ...
        cur = conn.execute(
            ...
            (item['skill_name'], item.get('parent_id'), item['title'],
             item.get('description', ''), item.get('level', 0), item.get('sort_order', 0), item.get('version', 1))
        )
...
def delete_skill_roadmap(id):
    ...
    def _delete_children(parent_id):
        children = conn.execute('SELECT id FROM skill_roadmaps WHERE parent_id=?', (parent_id,)).fetchall()
        for c in children:
            _delete_children(c[0])
        conn.execute('DELETE FROM skill_roadmap_progress WHERE roadmap_id=?', (parent_id,))
...
def bulk_create_skill_roadmaps():
    ...
    for i, t in enumerate(roadmap_items):
        ...
        for j, child in enumerate(t.get('children', [])):
            cur2 = conn.execute(
                'INSERT INTO skill_roadmaps (skill_name, parent_id, title, description, level, sort_order, version) VALUES (?, ?, ?, ?, ?, ?, ?)',
                (skill, new_id, child['title'], child.get('description', ''), child.get('level', t.get('level', 0)), j, version)
            )
...
def get_all_skill_progress():
    ...
    for (skill_name,) in skills:
        ...
        src_row = conn.execute('SELECT source FROM tech_stack WHERE name=?', (skill_name,)).fetchone()
...
# ═══════════════════════════════════════════════════════
# Skill Topics Generation — background worker
# ═══════════════════════════════════════════════════════
...
def _update_skill_progress(skill, job_type='generate', **kwargs):
    ...
    if row:
        ...
        if fields:
            values.append(row[0])
            conn.execute(f"UPDATE skill_roadmap_jobs SET {','.join(fields)} WHERE id=?", values)
    else:
        cols = ['skill_name', 'job_type'] + list(kwargs.keys())
        vals = [skill, job_type] + list(kwargs.values())
        placeholders = ','.join(['?'] * len(vals))
        conn.execute(f"INSERT INTO skill_roadmap_jobs ({','.join(cols)}) VALUES ({placeholders})", vals)
    conn.commit()
    conn.close()
...
def _run_generate_worker(skill):
    ...
    _update_skill_progress(skill, status='running', step=1, message='Preparing prompt', started_at=_datetime.now().isoformat(), error=None)
    try:
        # Step 1: Get level
        _update_skill_progress(skill, step=1, message='Reading skill level')
        conn = get_db()
        row = conn.execute('SELECT level FROM tech_stack WHERE name=?', (skill,)).fetchone()
        conn.close()
        ...
        prompt = load_prompt('skill_roadmaps', project_root=PROJECT_ROOT, tmp_dir=TMP_DIR, pid=0,
                             ...
                             checked_topics='none', growth_context='',
        ...
        if proc.returncode == 0 and _os.path.exists(result_file):
            ...
            for i, t in enumerate(roadmap_data):
                ...
                for j, child in enumerate(t.get('children', [])):
                    conn.execute(
                        'INSERT INTO skill_roadmaps (skill_name, parent_id, title, description, level, sort_order, version) VALUES (?, ?, ?, ?, ?, ?, ?)',
                        (skill, parent_id, child['title'], child.get('description', ''), child.get('level', t.get('level', 0)), j, new_version)
                    )
            conn.commit()
            conn.close()
        ...
        _update_skill_progress(skill, status='failed', error=str(e), completed_at=_datetime.now().isoformat())
...
def generate_skill_roadmaps():
    ...
    if current_status in ('running', 'queued'):
        return jsonify({'status': 'already_running'}), 409
    _update_skill_progress(skill, status='queued', step=0, message='Queued', started_at=None, completed_at=None, error=None)
...
def extend_skill_roadmaps():
    ...
    if current_status in ('running', 'queued'):
        return jsonify({'status': 'already_running'}), 409
    _update_skill_progress(skill, job_type='extend', status='queued', step=0, message='Queued', started_at=None, completed_at=None, error=None)
...
def finegrain_skill_roadmaps():
    ...
    if current_status in ('running', 'queued'):
        return jsonify({'status': 'already_running'}), 409
    _update_skill_progress(skill, job_type='finegrain', status='queued', step=0, message='Queued', started_at=None, completed_at=None, error=None)
    t = _threading.Thread(target=_run_grow_worker, args=(skill, 'finegrain'), daemon=True)
    t.start()
    return jsonify({'status': 'started', 'skill': skill})
...
def cancel_skill_gen():
    ...
    _update_skill_progress(skill, status='cancelled', message='Cancelled by user', completed_at=_datetime.now().isoformat())
...
def _build_roadmap_tree_for_prompt(rows):
    ...
    for r in rows:
        ...
        if pid and pid in by_id:
            ...
            roots.append(r)
    return [{'title': r['title'], 'description': r.get('description', ''),
             'children': [{'title': c['title'], 'description': c.get('description', '')} for c in r['children']]}
            for r in roots]
