"""SQLAlchemy-based preference repository implementation."""

from typing import Any

from sqlalchemy.orm import Session

from career.domain.repositories.preference_repository import IPreferenceRepository
...
class SQLAlchemyPreferenceRepository(IPreferenceRepository):
    """SQLAlchemy implementation of preference repository."""

    def __init__(self, session: Session):
        self._session = session

    def _to_dict(self, m: PreferenceModel) -> dict[str, Any]:
        return {
            "id": m.id,
            "category": m.category,
    ...
    def get_all(self) -> list[dict[str, Any]]:
        rows = self._session.query(PreferenceModel).order_by(PreferenceModel.priority.desc()).all()
        return [self._to_dict(r) for r in rows]

    def get_by_id(self, pref_id: int) -> dict[str, Any] | None:
        m = self._session.query(PreferenceModel).filter(PreferenceModel.id == pref_id).first()
        return self._to_dict(m) if m else None

    def get_enabled_by_scopes(self, scopes: list[str]) -> list[dict[str, Any]]:
        rows = self._session.query(PreferenceModel).filter(
            PreferenceModel.enabled == 1,
            PreferenceModel.scope.in_(scopes),
        ).order_by(PreferenceModel.priority.desc()).all()
        return [self._to_dict(r) for r in rows]
    ...
    def bulk_update(self, items: list[dict[str, Any]]) -> int:
        ...
        for item in items:
            if "id" not in item:
                continue
            m = self._session.query(PreferenceModel).filter(PreferenceModel.id == item["id"]).first()
            if not m:
                continue
