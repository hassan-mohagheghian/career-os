...
class TestTechStackCoreLogic:
    ...
    def test_get_skills_with_aliases(self, test_db):
        conn = sqlite3.connect(test_db)
        conn.execute("INSERT INTO tech_stack (name, level) VALUES (?, ?)", ('PostgreSQL', 4))
        conn.execute("INSERT INTO skill_aliases (skill_id, alias_name, normalized_name) VALUES (?, ?, ?)",
            (1, 'Postgres', 'postgres'))
        conn.commit()

        rows = conn.execute("SELECT * FROM tech_stack WHERE hidden=0").fetchall()
    ...
    def test_create_duplicate_skill(self, test_db):
        conn = sqlite3.connect(test_db)
        conn.execute("INSERT INTO tech_stack (name, level) VALUES (?, ?)", ('Python', 4))
        conn.commit()

        existing = conn.execute("SELECT id FROM tech_stack WHERE name=?", ('Python',)).fetchone()
        assert existing is not None
        conn.close()

    def test_rename_skill_updates_all_tables(self, test_db):
        conn = sqlite3.connect(test_db)
        conn.execute("INSERT INTO tech_stack (name, level) VALUES (?, ?)", ('Postgres', 4))
        ...
        new_name = 'PostgreSQL'
        conn.execute("UPDATE tech_stack SET name=? WHERE id=?", (new_name, 1))
        ...
        assert conn.execute("SELECT name FROM tech_stack WHERE id=1").fetchone()[0] == 'PostgreSQL'
    ...
    def test_delete_skill_removes_aliases(self, test_db):
        ...
        conn.execute("DELETE FROM tech_stack WHERE id=?", (1,))
        conn.commit()

        assert conn.execute("SELECT COUNT(*) FROM tech_stack").fetchone()[0] == 0
        assert conn.execute("SELECT COUNT(*) FROM skill_aliases").fetchone()[0] == 0
        conn.close()

    def test_hide_and_restore_skill(self, test_db):
        conn = sqlite3.connect(test_db)
        conn.execute("INSERT INTO tech_stack (name, level, hidden) VALUES (?, ?, ?)", ('Python', 4, 0))
        conn.commit()

        conn.execute("UPDATE tech_stack SET hidden=1 WHERE id=1")
        conn.commit()
        assert conn.execute("SELECT hidden FROM tech_stack WHERE id=1").fetchone()[0] == 1

        conn.execute("UPDATE tech_stack SET hidden=0 WHERE id=1")
        conn.commit()
        assert conn.execute("SELECT hidden FROM tech_stack WHERE id=1").fetchone()[0] == 0
        conn.close()

    def test_merge_creates_alias_and_hides_source(self, test_db):
        conn = sqlite3.connect(test_db)
        conn.execute("INSERT INTO tech_stack (name, level) VALUES (?, ?)", ('PostgreSQL', 4))
        conn.execute("INSERT INTO tech_stack (name, level) VALUES (?, ?)", ('Postgres', 3))
        conn.commit()
        ...
        # Hide source
        conn.execute("UPDATE tech_stack SET hidden=1 WHERE id=?", (source_id,))
        conn.commit()

        alias = conn.execute("SELECT alias_name FROM skill_aliases WHERE skill_id=?", (target_id,)).fetchone()
        source = conn.execute("SELECT hidden FROM tech_stack WHERE id=?", (source_id,)).fetchone()
    ...
    def test_skill_categories(self, test_db):
        ...
        conn.execute("INSERT INTO tech_stack (name, level, category) VALUES (?, ?, ?)", ('Fintech', 2, 'domain'))
        conn.commit()
        ...
        domain = conn.execute("SELECT name FROM tech_stack WHERE category='domain'").fetchall()
