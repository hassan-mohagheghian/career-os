"""Tests for the FastAPI/socketio entrypoint (app/server/entrypoints/api.py).
...
from app.server.entrypoints import api


@pytest.fixture
def mock_sio():
    with patch('app.server.entrypoints.api.sio', AsyncMock()) as sio:
        yield sio
...
class TestCreateApp:
    ...
    def test_spa_static_serving(self, tmp_path):
        ...
        with patch('app.server.entrypoints.api.STATIC_FOLDER', str(tmp_path)):
            app = api.create_app()
        client = TestClient(app)
    ...
    def test_no_static_folder_no_fallback(self, tmp_path):
        with patch('app.server.entrypoints.api.STATIC_FOLDER', str(tmp_path / 'missing')):
            app = api.create_app()
        client = TestClient(app)
