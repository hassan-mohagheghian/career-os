...
def _job(sa_session, num=1, **kwargs):
    defaults = {
        "num": num,
...
class TestSourceFilter:
    ...
    def test_pagination_limit_and_offset(self, repo, sa_session):
        _job(sa_session, num=1)
        _job(sa_session, num=2)
        _job(sa_session, num=3)
        result = repo.get_all(limit=1, offset=1)
        assert result["total"] == 3
        assert len(result["items"]) == 1

    def test_sorting_desc_by_started_at(self, repo, sa_session):
        _job(sa_session, num=1, created_at="2026-07-27T10:00:00")
        _job(sa_session, num=2, created_at="2026-07-27T11:00:00")
        result = repo.get_all(source_filter="job-processing")
...
class TestDefaultTitles:
    def test_pending_job_without_company(self, repo, sa_session):
        _job(sa_session, num=1, company=None, status="imported")
        result = repo.get_all(source_filter="job-processing")
...
class TestContextQueries:
    def test_get_for_job(self, repo, sa_session):
        _job(sa_session, num=5, status="completed", company=None)
...
class TestGetActiveCount:
    def test_job_context(self, repo, sa_session):
        _job(sa_session, num=1, status="processing")
        _job(sa_session, num=2, status="queued")
        _job(sa_session, num=3, status="imported")
        assert repo.get_active_count("job", job_num=1) == 1
        assert repo.get_active_count("job", job_num=2) == 1
    ...
    def test_exception_returns_zero(self, repo):
        with patch(PATCH_TARGET, side_effect=RuntimeError("boom")):
            assert repo.get_active_count("job", job_num=1) == 0
