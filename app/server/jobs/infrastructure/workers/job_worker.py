...
from shared.infrastructure.process.models import PipelineStep
...
class JobWorker(WorkerBase):
    ...
    def _step_fetch(self, pid: int, item: dict) -> Optional[str]:
        ...
        from jobs.infrastructure.workers.worker import _fetch_url, _fetch_multi_source
        url = item.get('url', '')
    ...
    def _step_save(self, pid: int, item: dict, raw_text: str,
        ...
        from jobs.infrastructure.workers.worker import _insert_job, _insert_summary, _get_next_num, _get_existing_num
        url = item.get('url', '')
