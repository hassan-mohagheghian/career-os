...
def connect_db():
    ...
    conn = sqlite3.connect('/home/hassan/Subjects/Berlin/Job-Search/app/server/jobs.db')
...
def generate_strategy(jobs: List[Dict[str, Any]], skill_counts: Dict[str, int]) -> List[Dict[str, str]]:
    ...
    strategy.append({
        ...
        'description': f"Top matches: {top_5_list}. These have the highest compatibility with Hassan's profile."
    })
    ...
    strategy.append({
        'icon': '🐍',
        'title': 'Python Edge',
        'description': f"{python_jobs}/{len(jobs)} jobs require Python. Hassan's 9+ years gives strong competitive advantage in Berlin market."
    })
    ...
    if django_jobs > 0 or fastapi_jobs > 0:
        strategy.append({
            ...
            'description': f"{django_jobs} Django + {fastapi_jobs} FastAPI jobs. Hassan's expertise in both frameworks is highly valued."
        })
    
    return strategy

def generate_strengths_weaknesses(skill_counts: Dict[str, int], total_jobs: int) -> tuple:
    """Generate strengths and weaknesses based on Hassan's skills."""
    # Hassan's actual skills based on his profile
    hassan_skills = {
        'Python': True,  # 9+ years
        'Django': True,
        'FastAPI': True,
    ...
    for skill, count in skill_counts.items():
        ...
        if skill in hassan_skills:
            strengths.append({
                'name': skill,
                'detail': f"{count}/{total_jobs} jobs require {skill} — Hassan has strong experience"
            })
        else:
            weaknesses.append({
                'name': skill,
                'detail': f"{count}/{total_jobs} jobs require {skill} — Hassan has gaps or limited experience"
            })
...
def main():
    ...
    try:
        ...
        # Save to file
        output_path = '/home/hassan/Subjects/Berlin/Job-Search/data/dashboard_insights_0.json'
