...
class TestJobsEndpoints:
    ...
    def test_rescore_job(self, client, sa_session):
        ...
        with patch("core.queue.get_queue_manager") as mock_qm:
...
class TestCompaniesEndpoints:
    ...
    def test_list_companies_with_intelligence(self, client, sa_session):
        from infrastructure.database.models.company_model import CompanyModel, CompanyIntelligenceModel
    ...
    def test_get_company_links(self, client, sa_session):
        from infrastructure.database.models.company_model import CompanyModel, CompanyLinkModel
        co = CompanyModel(name="Google")
        sa_session.add(co)
        sa_session.commit()
...
class TestSkillRoadmapsEndpoints:
    ...
    def test_build_roadmap_tree(self):
        from api.v1.skill_roadmaps import build_roadmap_tree
        rows = [
...
class TestRouterCompatRoutes:
    ...
    def test_skill_roadmap_progress_compat_by_skill(self, client, sa_session):
        from infrastructure.database.models.misc_models import SkillRoadmapModel, SkillRoadmapProgressModel
        rm = SkillRoadmapModel(skill_name="Python", title="Basics")
        sa_session.add(rm)
        sa_session.commit()
    ...
    def test_add_pending_company_link_compat(self, client):
        r = client.post("/api/pending-companies/1/links", json={"url": "https://example.com"})
        assert r.status_code == 200
