...
import { ArrowLeft, LinkSimple, Eye, PencilSimple } from '@phosphor-icons/react'
...
export function WorkspaceHeader({ job, applicationStatus, onViewDetails, onEdit }: WorkspaceHeaderProps) {
  return (
      ...
      <div className="flex items-center justify-between gap-4 mb-3">
        ...
        <div className="flex items-center gap-2">
          {onViewDetails && (
            ...
            >
              <Eye className="w-3.5 h-3.5" /> Job Detail
            </Button>
          )}
          {onEdit && (
            <Button
              ...
              aria-label="Job Edit"
            >
              <PencilSimple className="w-3.5 h-3.5" /> Job Edit
            </Button>
          )}
          {job.url && (
            <a
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {job.company_id && job.company_name ? (
              <Link
                href={`/companies?company=${encodeURIComponent(job.company_id)}`}
            ...
            {job.company_type && (
              <Badge variant="secondary" className="ml-1 align-middle text-2xs">
