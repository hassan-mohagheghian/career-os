...
import type { JobDetail, JobEditInput, JobNoteItem, JobLinkItem } from '@/entities/job/types'
        ...
        <ScrollArea className="flex-1 min-h-0">
          ...
          {!loading && detail && (
            <div className="space-y-3 px-4 py-4">
              <Field label="Title">
                <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Staff Software Engineer" />
              </Field>
              <Field label="Role">
              ...
              <Field label="Job Post URL" required hint={(!urlValid && url ? 'URL must start with http:// or https://' : undefined)}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Visa">
                  <Input value={visa} onChange={(e) => setVisa(e.target.value)} placeholder="Strong" />
                </Field>
                <Field label="Salary">
                  <Input value={salary} onChange={(e) => setSalary(e.target.value)} placeholder="€90k - €110k" />
                </Field>
              </div>
              ...
              <Field label="Notes">
                <div className="space-y-1.5">
                  {notes.map((n, i) => (
                    <div key={i} className="flex items-start gap-1 rounded border bg-muted/50 px-2 py-1 text-2xs">
                      <div className="flex-1 min-w-0">
                        {n.title && <div className="text-muted-foreground truncate">{n.title}</div>}
                        <div className="break-all whitespace-pre-wrap">{n.content}</div>
                      </div>
                      <button onClick={() => setNotes(notes.filter((_, j) => j !== i))} className="shrink-0 text-muted-foreground hover:text-destructive">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
                  <div className="flex gap-1">
                    <Textarea value={noteDraft} onChange={(e) => setNoteDraft(e.target.value)} placeholder="Add a note..." className="h-8 min-h-0 text-xs resize-none flex-1" />
                    <Button type="button" size="sm" variant="outline" className="h-8 px-2 shrink-0" disabled={!noteDraft.trim()}
                      aria-label="Add note"
                      onClick={() => { setNotes([...notes, { content: noteDraft.trim() }]); setNoteDraft('') }}>
              ...
              <Field label="Links">
                <div className="space-y-1.5">
                  {links.map((l, i) => (
                    <div key={i} className="flex items-start gap-1 rounded border bg-muted/50 px-2 py-1 text-2xs">
                      <div className="flex-1 min-w-0">
                        <a href={l.url} target="_blank" rel="noreferrer" className="text-primary hover:underline break-all">{l.url}</a>
                        {l.title && <div className="text-muted-foreground truncate">{l.title}</div>}
                      </div>
                      <button onClick={() => setLinks(links.filter((_, j) => j !== i))} className="shrink-0 text-muted-foreground hover:text-destructive">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
                  <div className="space-y-1">
                    <div className="flex gap-1">
                      <Input type="url" value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} placeholder="Link URL (https://...)" className="h-8 flex-1" />
                      <Input value={linkTitle} onChange={(e) => setLinkTitle(e.target.value)} placeholder="Title (optional)" className="h-8 flex-1" />
                      <Button type="button" size="sm" variant="outline" className="h-8 px-2 shrink-0"
                        disabled={!linkUrl.trim() || !linkUrl.trim().startsWith('http')}
                        aria-label="Add link"
                        onClick={() => { setLinks([...links, { url: linkUrl.trim(), title: linkTitle.trim() || null }]); setLinkUrl(''); setLinkTitle('') }}>
                        <Plus className="w-3 h-3" />
                      </Button>
                    </div>
                    {linkUrl && !linkUrl.startsWith('http') && (
                      <p className="text-2xs text-destructive">URL must start with http:// or https://</p>
                    )}
                  </div>
                </div>
              </Field>
              ...
              <Field label="Description">
                <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Job description..." className="min-h-[80px] text-xs resize-none" />
              </Field>
