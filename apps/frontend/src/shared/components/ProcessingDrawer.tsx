...
export function ProcessingDrawer({ open, onOpenChange, reloadKey, targetType = 'job' }: ProcessingDrawerProps) {
  ...
  const matchesTarget = useCallback((entry: QueueEntry) => {
  ...
  }, [open, targetType, loadSnapshot, loadWorkflow, ensureWorkflow, setWorkflow, clearWorkflow])
      ...
      <SheetContent side="right" className="job-drawer w-[400px] sm:w-[480px] p-0 flex flex-col">
        ...
        <ScrollArea className="flex-1 min-h-0 min-w-0">
          <div className="py-2 space-y-4 min-w-0">
            <QueueSection title="Running" items={filtered.processing} color="text-emerald-500" emptyText={emptyText} renderDetail={renderDetail} renderActions={processingActions} />
            <QueueSection title="Waiting" items={filtered.queued} color="text-blue-500" emptyText={emptyText} renderDetail={renderDetail} renderActions={queuedActions} />
            <QueueSection title="Failed" items={filtered.failed} color="text-red-500" emptyText={emptyText} renderDetail={renderDetail} renderActions={failedActions} />
          </div>
        </ScrollArea>
      </SheetContent>
    </Sheet>
