...
def test_list_pending_jobs(client, sa_session):
    _mark_all_jobs_processed(sa_session)
    sa_session.add(JobModel(num=601, url="https://example.com/p1", status="created"))
    sa_session.add(JobModel(num=602, url="https://example.com/p2", status="processed"))
    sa_session.commit()
    resp = client.get("/api/pending")
    assert resp.status_code == 200
    assert [r["num"] for r in resp.json()] == [601]


def test_create_pending_job(client, sa_session):
    resp = client.post(
        "/api/pending", json={"url": "https://example.com/newjob1", "source": "test"}
    )
    assert resp.status_code == 200
    data = resp.json()
    assert data["status"] == "created"
    assert data["url"] == "https://example.com/newjob1"


def test_process_all_pending_jobs_empty(client, sa_session):
    _mark_all_jobs_processed(sa_session)
    with patch("shared.infrastructure.taskiq.client.enqueue_job_sync") as mock_enq:
        resp = client.post("/api/pending/process-all")
    assert resp.status_code == 200
    assert resp.json() == {"status": "queued", "count": 0}
    mock_enq.assert_not_called()


def test_process_all_pending_jobs_with_items(client, sa_session):
    _mark_all_jobs_processed(sa_session)
    sa_session.add(JobModel(num=603, url="https://example.com/pa1", status="created"))
    sa_session.add(JobModel(num=604, url="https://example.com/pa2", status="created"))
    sa_session.commit()
    with patch("shared.infrastructure.taskiq.client.enqueue_job_sync") as mock_enq:
        resp = client.post("/api/pending/process-all")
    assert resp.status_code == 200
    ...
    assert {c.args[0] for c in mock_enq.call_args_list} == {603, 604}


def test_get_pending_job_found(client, sa_session):
    sa_session.add(JobModel(num=701, url="https://example.com/g1", status="created"))
    sa_session.commit()
    resp = client.get("/api/pending/701")
    assert resp.status_code == 200
    assert resp.json()["num"] == 701


def test_get_pending_job_not_found(client, sa_session):
    resp = client.get("/api/pending/900002")
    assert resp.status_code == 404
    assert resp.json() == {"error": "Not found"}


def test_delete_pending_job_found(client, sa_session):
    sa_session.add(JobModel(num=702, url="https://example.com/d1", status="created"))
    sa_session.commit()
    resp = client.delete("/api/pending/702")
    assert resp.status_code == 200
    assert resp.json() == {"status": "deleted"}
    job = sa_session.query(JobModel).filter(JobModel.num == 702).first()
    assert job.deleted == 1


def test_delete_pending_job_not_found(client, sa_session):
    resp = client.delete("/api/pending/900003")
    assert resp.status_code == 200
    assert resp.json() == {"error": "Not found"}


def test_reset_pending_job(client, sa_session):
    sa_session.add(JobModel(num=703, url="https://example.com/r1", status="processing"))
    sa_session.commit()
    resp = client.post("/api/pending/703/reset")
    assert resp.status_code == 200
    assert resp.json() == {"status": "reset"}
    job = sa_session.query(JobModel).filter(JobModel.num == 703).first()
    assert job.status == "created"
    assert job.error is None


def test_process_pending_job(client, sa_session):
    sa_session.add(JobModel(num=704, url="https://example.com/proc1", status="created"))
    sa_session.commit()
    with patch("shared.infrastructure.taskiq.client.enqueue_job_sync") as mock_enq:
        resp = client.post("/api/pending/704/process")
    assert resp.status_code == 200
    assert resp.json() == {"status": "queued"}
    mock_enq.assert_called_once_with(704)
