"""ValidateContextNode — validates that enough company information exists.

Valid: at least one meaningful content source exists.
Invalid: no extracted content, empty notes, or no usable source.

Delegates to CompanyContextValidatorService. Conditional edges route the
...
class ValidateContextNode:
    ...
    def __call__(self, state: CompanyProcessingState) -> CompanyProcessingState:
        ...
        context = state.processing_context or CompanyProcessingContext(
