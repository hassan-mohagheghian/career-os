...
Loads the user's skills, the latest resume raw text, and the enabled scoring
rules, and stores formatted text in the state's analysis_context.
"""
...
class PrepareProfileNode:
    ...
    def __call__(self, state: JobProcessingState) -> JobProcessingState:
        ...
        try:
            ...
            resume_raw = self._resumes.get_latest_original_raw_text() or self._resumes.get_latest_linkedin_raw_text()
        except Exception as e:
            state.errors.append(f"[{NODE_ID}] Failed to load user profile: {e}")
            skills, rule_rows, resume_raw = [], [], None
