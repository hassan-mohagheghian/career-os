...
(`APPLICATION_RESUME` / `APPLICATION_COVER_LETTER` executions), a single-graph
workflow that reuses the job analysis, company and candidate context builders
(no re-analysis). See `docs/ai/application-intelligence.md`.

Job-preparation roadmaps are generated by the parallel `RoadmapGenerationGraph`
(`ROADMAP_GENERATION`). See `docs/ai/roadmap-generation.md`.
